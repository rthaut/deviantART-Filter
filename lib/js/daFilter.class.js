...
class deviantARTFilter {
    ...
    manage(event) {
        ...
        this.filters.forEach(function (filter) {
            ...
            //@TODO this needs to be updated whenever the list is modified...
            var listTableCount = $('<p/>')
                .addClass('filter-data-table-info')
                .html('Current Filtered Users Count: <strong>' + filter.list.data.length + '</strong>')
                .appendTo(listContent);

            var listTableLoading = $('<p/>')
                .addClass('filter-data-table-loading')
                .text('Cleaning filtered ' + filter.label.toLowerCase() + ' data...')
                .appendTo(listContent);

            setTimeout(function () {
                filter.list.cleanAsync().then(function () {
                    listTableLoading.text('Loading filtered ' + filter.label.toLowerCase() + ' data...');
                    return filter.getTableAsync();
                }).then(function (table) {
