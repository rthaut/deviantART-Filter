...
class deviantARTFilter {
    ...
    addCustomEventListeners() {
        ...
        window.addEventListener('daFilter-list-saved', function (e) {
            console.log('Received Custom Event "daFilter-list-saved"', e);
            self.filters.forEach(function (filter) {
                if (filter.list.listObjectName === e.detail.listObjectName) {
                    self.refreshFilterTableInfo(filter);
                }
            });
        }, false);

        window.addEventListener('daFilter-list-cleaned', function (e) {
            console.log('Received Custom Event "daFilter-list-cleaned"', e);
            self.filters.forEach(function (filter) {
                if (filter.list.listObjectName === e.detail.listObjectName) {
        ...
        window.addEventListener('daFilter-list-reset', function (e) {
            console.log('Received Custom Event "daFilter-list-reset"', e);
            self.filters.forEach(function (filter) {
                if (filter.list.listObjectName === e.detail.listObjectName) {
                    self.refreshFilterTable(filter);
                }
        ...
        window.addEventListener('daFilter-imported', function (e) {
            console.log('Received Custom Event "daFilter-imported"', e);
            self.filters.forEach(function (filter) {
                if (filter.label === e.detail.label) {
                    self.refreshFilterTable(filter);
                }
            });
        }, false);
    ...
    manage(event) {
        ...
        this.filters.forEach(function (filter) {
            ...
            $('<p/>').html('Current Filtered ' + filter.label + ' Count: <strong>' + filter.list.data.length + '</strong>')
