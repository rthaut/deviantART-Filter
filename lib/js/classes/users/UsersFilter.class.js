/* global Filter, UserObject, UserList */
...
class UsersFilter extends Filter {
     ...
     */
    create(properties) {
        return new UserObject(properties);
    }
    ...
    getTable() {
        ...
        for (var i = 0; i < this.list.data.length; i++) {
            table.append(this.getTableRow(this.list.data[i]));
        }
    ...
    /**
     * Builds the table for managing users asynchronously
     * @returns {Element}
     */
    getTableAsync() {
        console.log(this.label + '.getTableAsync()');

        var self = this;
        var promise = new Promise(function (resolve, reject) {
            try {
                var table = self.getTable();
                console.log(self.label + '.getTableAsync() :: Resolve', table);
                resolve(table);
            } catch (error) {
                console.error(error);
                console.log(self.listObjectName + '.getTableAsync() :: Reject', error.message);
                reject(error.message);
            }
        });

        console.log(this.label + '.getTableAsync() :: Return', promise);
        return promise;
    }

    getTableRow(filterObject) {
        var row = $('<tr/>');

        // username column w/ link
        if (typeof filterObject.username !== 'undefined' && filterObject.username !== null) {
            row.append('<td><a class="external" href="http://' + filterObject.username + '.deviantart.com/" target="_blank">' + filterObject.username + '</a></td>');
        } else {
            row.append('<td>---</td>');
        }
        ...
        $('<button/>')
            ...
            .attr('username', filterObject.username)
            .html('Unhide User')
    ...
    hideButtonClickHandler(event) {
        ...
        if (this.hide(user)) {
            $('input#username').parents('table').append(this.getTableRow(user));
            $('input#username').val('');
        }
