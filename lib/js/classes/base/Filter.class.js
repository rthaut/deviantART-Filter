/* global FilterObject, FilterList */
...
class Filter {
    ...
    import(data) {
        ...
        var promise = new Promise(function (resolve, reject) {
            ...
            try {
                ...
                console.error(error);
                console.log(self.label + '.import() :: Reject', error.message);
            ...
            var event = new CustomEvent('daFilter-imported', { 'detail': { 'label': self.label } });
    ...
    apply() {
        ...
        var event = new CustomEvent('daFilter-applied', { 'detail': { 'label': this.label } });
        window.dispatchEvent(event);
    ...
    cleanFilterListClickEventHandler(event) {
        ...
        console.log(this.label + '.cleanFilterListClickEventHandler() :: Complete');
    }
    ...
    resetFilterListClickEventHandler(event) {
        ...
        daDialog.confirm(text, title, function (confirmed) {
            console.log(self.label + '.resetFilterListClickEventHandler() :: Callback :: Confirmed', confirmed);
            if (confirmed) {
                self.list.reset();
                self.apply();
                daDialog.alert('All of your Filtered ' + self.label + ' have been reset successfully.')
            }
        });
