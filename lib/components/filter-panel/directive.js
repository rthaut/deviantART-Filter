        ...
        $scope.removeItemFromFilter = function ($event, filter, item, $index) {
            ...
            browser.runtime.sendMessage({
                ...
                if (response) {
                    ...
                    $scope.$apply(() => {
                        filter.total--;
                        //@TODO this isn't reliable if multiple removes are triggered in rapid succession
                        //      ideally the response would just include all new data for the filter
                        //      this would also ensure the first item from the next "page" was "brought forward"
