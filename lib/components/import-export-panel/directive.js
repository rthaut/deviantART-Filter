...
angular.module('deviantArtFilter.components.ImportExportPanel', ['ui.bootstrap'])
    ...
    .controller('ImportExportPanelCtrl', ['$scope', function ($scope) {
        ...
        $scope.exportFilterData = function ($event) {
            ...
            $event.target.textContent = 'Generating Filter Export File...';   //@TODO i18n
            ...
            browser.runtime.sendMessage({
                ...
                link.download = `${filename}-${date.getUTCFullYear()}-${date.getMonth() + 1}-${date.getDate()}-${Math.round(date.getTime() / 1000)}.json`;
                link.dispatchEvent(new MouseEvent('click'));
            }).catch((error) => {
                $scope.$apply(() => {
                    $scope.alerts.push({
                        'type': 'danger',
                        'msg': 'An error occurred while exporting filter data to file' + (error.message ? `: ${error.message}` : '')   //@TODO i18n
                    });
                });
            }).then(() => {
                $scope.$apply(() => {
                    $event.target.removeAttribute('disabled');
                    $event.target.textContent = 'Export Filter Data';   //@TODO i18n
                });
            });
        };
        ...
        $scope.importFilterData = function (file) {
            ...
            browser.runtime.sendMessage({
                ...
                $scope.$apply(() => {
                    $scope.alerts.push({
                        'type': 'danger',
                        'msg': 'An error occurred while importing filter data from file' + (error.message ? `: ${error.message}` : '')   //@TODO i18n
                    });
                    $scope.importing = false;
                });
            });
