const Filter = (() => {
    ...
    class Filter {
         ...
         * @param {Node[]} newThumbs - The thumbs to filter and/or modify
         */
        handleThumbs(newThumbs) {
            console.log(`[Content] Filter('${this.name}').handleThumbs()`, newThumbs);
            ...
            try {
                this.filterThumbs(newThumbs);
                this.updateThumbs(newThumbs);
            } catch (error) {
                console.error(error);
            }
        }
