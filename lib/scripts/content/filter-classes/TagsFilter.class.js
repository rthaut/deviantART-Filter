    ...
    class TagsFilter extends Filter {
        ...
        updateFilter(filter) {
            ...
            filter.data.forEach((item) => {
                ...
                this.styleSheet.insertRule(`body.placeholders ${thumb} a.torpedo-thumb-link[data-tags${selector}="${item.tag}"]::before { ${placeholder} }`);
                this.styleSheet.insertRule(`body.placeholders ${thumb} a.torpedo-thumb-link[data-tags${selector}="${item.tag}"]::after { content: "${text}"; ${placeholder_text} }`);

                // hide the info panel that displays on mouseover when using placeholders
                this.styleSheet.insertRule(`body.placeholders ${thumb} a.torpedo-thumb-link[data-tags${selector}="${item.tag}"] + span.info { ${invisible} }`);

                // hide the user toggle corner icon for thumbs filtered by a tag to avoid confusion
                this.styleSheet.insertRule(`body.placeholders ${thumb} a.torpedo-thumb-link[data-tags${selector}="${item.tag}"] span.hide-user-corner { ${invisible} }`);
            });
        }
    }
