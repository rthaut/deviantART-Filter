...
const BrowserRuntime = (() => {
    ...
    class BrowserRuntime {
        ...
        onRuntimeMessage(request, sender, sendResponse) {
            console.log('[Background] BrowserRuntime.onRuntimeMessage()', request, sender, sendResponse);
            ...
            switch (request.action) {
                ...
                case 'import-filters':
                    return Filters.importFiltersFromFile(request.data.file);

                case 'export-filters':
                    return Filters.exportFilters();

                case 'migrate-filter':
                    return Migration.migrateFilter(request.data.filter, request.data.items);

                case 'get-category-hierarchy':
                    return Metadata.getCategoryHierarchy(request.data);
