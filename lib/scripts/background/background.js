    ...
    return browser.tabs.query({ 'url': '*://*.deviantart.com/*' }).then((tabs) => {
        tabs.forEach(function (tab) {
            browser.pageAction.setPopup({
                tabId: tab.id,
                popup: enabled ? browser.extension.getURL('pages/popup/deviantart-filter.popup.html') : ''
            });
        }, this);
        return true;
    });
        ...
        switch (res.managementPanelType) {
            case 'window':
                return browser.windows.create({
                    url: browser.extension.getURL('pages/manage/deviantart-filter.manage.html'),
                    type: 'popup',
                    width: 800,
                    height: 600
                });
