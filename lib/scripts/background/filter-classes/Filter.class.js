...
const Filter = (() => {
    ...
    class Filter {
        ...
        /**
         * Applies the CSS for the filter and loads additional data as needed
         * @param {tab} tab
         */
        applyFilter(tab) {
            console.log(`${this.name}.applyFilter()`, tab);

            return Promise.all([
                this.applyFilterCSS(tab),
                this.loadFilterData(tab)
            ]);
        }
         ...
         */
        loadFilterData(tab) {
            console.log(`${this.name}.loadFilterData()`, tab);
