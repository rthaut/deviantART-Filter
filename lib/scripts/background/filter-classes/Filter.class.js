...
const Filter = (() => {
    ...
    class Filter {
        ...
        /**
         * Get the CSS to hide the filterable item
         * @param {object} item - Object represneting a filterable item
         * @returns {string} - The CSS to hide the filterable item
         */
        getCSS(item) {
            console.log(`${this.name}.getCSS()`);
        ...
        exists(item) {
            ...
            return this.getFilterData().then((items) => {
                return (Utils.FindObjectInArray(item, items, Object.keys(this.properties.required)) >= 0);
            });
        }
        ...
        import(newItems) {
            ...
            let properties = Object.keys(this.properties.required);
            ...
            return this.getFilterData().then((items) => {
                ...
                for (let item in newItems) {
                    ...
                    let idx = Utils.FindObjectInArray(item, items, properties);
                    if (idx >= 0) {
                        results.duplicate++;
                        continue;
                    }
        ...
        add(item, limit, offset) {
            ...
            if (!this.isValid(item)) {
                throw new Error(`${label} is invalid`);    //@TODO i18n
            }

            return this.getFilterData().then((items) => {
                let idx = Utils.FindObjectInArray(item, items, properties);
                if (idx >= 0) {
                    throw new Error(`${label} is already filtered`);    //@TODO i18n
                }
        ...
        remove(item, limit, offset) {
            ...
            return this.getFilterData().then((items) => {
                let idx = Utils.FindObjectInArray(item, items, properties);
                if (idx < 0) {
                    throw new Error(`${label} is not filtered`);    //@TODO i18n
                }
