        ...
        isValid(item) {
            ...
            if (this.properties && this.properties.required) {
                this.properties.required.forEach((prop) => {
                    if (!item[prop.field] || !item[prop.field].length) {
                        console.log(`Required property "${prop.title}" is missing or empty`);
                        valid &= false;
                    }

                    const regex = new RegExp(`^${prop.pattern}$`);
                    console.log('RegExp:', regex);
                    console.log('Value:', item[prop.field]);
                    if (!regex.test(item[prop.field])) {
