...
const Metadata = (() => {
    ...
    const Metadata = {
         ...
         * @param {tab} tab
         */
        'getMetadataForTab': async function (tab) {
            ...
            let response;
            do {
                response = await DeviantArtAPI.GET(request.resource, request.data);
                _deviations = _deviations.concat(response.results.slice());
                request.data.offset = response.next_offset;
            } while (response && response.has_more && request.data.offset < REQUEST_DEFAULTS.limit);
